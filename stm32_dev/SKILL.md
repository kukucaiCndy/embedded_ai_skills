...
> **AI 必须向用户确认以下硬件信息，不能假设任何默认值。**

**确认清单：**

| 询问项 | 选项 | 说明 |
|--------|------|------|
| 芯片型号（丝印） | F103C8T6 / F103CBT6 / ... | 决定 CMSIS 类型（F103xB 等） |
| ST-Link 类型 | 原装 / 克隆版 | 影响烧录工具和驱动选择 |
| ST-Link 引脚数 | **4 针**（SWDIO/SWCLK/3V3/GND） / **6 针**（含 NRST） | 决定能否用硬件复位 |
| 开发板 LED 引脚 | PC13 / PA5 / ... | 影响测试代码 |
| 外部晶振 | 8MHz / 无 | 影响时钟配置（HSE vs HSI） |
| BOOT0/BOOT1 跳线帽 | 有 / 无 | 影响救砖方式 |

> 💡 **本 SKILL 中的具体操作（Zadig 驱动、BOOT0 救砖、4 针无 NRST 流程）均来自特定硬件的调试经验，仅供参考。AI 必须根据用户的实际硬件情况调整每一步。**

### 第四步：询问需求

> **AI**: 环境就绪（路径 A/B）。请问你想做什么？
> - [1] 创建新项目
> - [2] 编译当前项目
> - [3] 烧录到芯片
> - [4] 重新检查环境
